...
use svsm::mm::address_space::{SVSM_GLOBAL_BASE, SVSM_GLOBAL_BASE_END};
...
fn load_kernel_elf(
    ...
    let heap_alignment_padding = PAGE_SIZE_2M - PAGE_SIZE;
    let upper_bound = SVSM_GLOBAL_BASE_END.as_usize()
        - kernel_region_len
    ...
    let kaslr_offset = match get_kaslr_offset(kaslr_max_size as u64) {
        ...
        None => {
            log::error!("Error retrieving kaslr");
            0
        }
    };
